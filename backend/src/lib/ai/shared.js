...
import { GEMINI_API_KEY, GEMINI_MODEL, ORS_API_KEY } from "../../config/env.js";
// Re-exported so ai/* modules import config keys from one place.
// GEMINI_MODEL: used by classify, curate, grounding, poi-select (stubbed in tests).
// ORS_API_KEY: re-exported, consumed by pipeline.js.
export { GEMINI_MODEL, ORS_API_KEY };

// Shared Gemini client (null if no API key). Used by classify, curate,
// grounding, poi-select — they no-op/fallback when it's null.
export const genai = GEMINI_API_KEY
  ? new GoogleGenAI({ apiKey: GEMINI_API_KEY })
  : null;

// Used as systemInstruction by classify.js + curate.js.
// Anti-prompt-injection guard scoping the model to <user_request> tags.
...
// Used by classify.js + curate.js to force output language (en/lt).
...
// Used by classify.js + pipeline.js — default search theme per activity
// profile when the user gives no preferences.
...
// Used by sanitizePromptInput.
// Regexes flagging common prompt-injection phrasing in user input.
...
// Used by classify.js + curate.js.
// Strips control chars, trims/caps length, logs a warning on suspected
// injection (does not block — returns the cleaned string).
...
// Used by curate.js, grounding.js, poi-select.js (and stubbed in tests).
// Tolerant JSON parse: strips ```json fences, falls back to first [..] slice.
...
// Used by pipeline.js.
// De-duplicates POIs by place_id (or ~5-decimal lat/lng key), dropping
// entries with no usable coords.
